...
func main() {
	...
	if err := processAEC(*micFile, *speakerFile, *outputFile); err != nil {
...
func processAEC(micPath, speakerPath, outputPath string) error {
	// Open input files
	...
	for {
		...
		// Perform AEC + NS processing
		outputPcmFrame := aec.ProcessFrame(micPcmFrame, speakerPcmFrame)
